...
bool PathTracker::initializeParams(std_srvs::Empty::Request& req, std_srvs::Empty::Response& res) {
    ...
    if (p_active_ != prev_active) {
        if (p_active_) {
            ...
            goal_sub_ = nh_.subscribe("move_base_simple/goal", 5, &PathTracker::Goal_Callback, this);
...
void PathTracker::Switch_Mode(MODE next_mode) {
    ...
    switch(working_mode_pre_) {
        case MODE::IDLE: {
            ROS_INFO("From: IDLE");
            break;
        }
        case MODE::GLOBAL_PATH_RECEIVED: {
            ROS_INFO("From: GLOBAL_PATH_RECEIVED");
            break;
        }
        case MODE::TRACKING: {
            ROS_INFO("From: TRACKING");
            break;
        }
        case MODE::TRANSITION: {
            ROS_INFO("From: TRANSITION");
            break;
        }
    }
    switch(working_mode_) {
        ...
        case MODE::GLOBAL_PATH_RECEIVED: {
            ROS_INFO("to: GLOBAL_PATH_RECEIVED");
            break;
        }
        case MODE::TRACKING: {
            ROS_INFO("to: TRACKING");
            break;
        }
        case MODE::TRANSITION: {
            ROS_INFO("to: TRANSITION");
            break;
        }
    }
